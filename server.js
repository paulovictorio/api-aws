...
//#region MongoDB - CRUD de usuários
...
app.post('/buckets/:bucketName/upload', async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Endpoint de upload - implementar com multer' });
    } catch (error) {
        logError("Erro ao efetuar upload", req, error);
        res.status(500).json({ error: 'Erro ao efetuar upload', details: error });
    }
});
...
app.delete('/buckets/:bucketName/file/:fileName', async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Endpoint de remoção - implementar remoção do objeto' });
    } catch (error) {
